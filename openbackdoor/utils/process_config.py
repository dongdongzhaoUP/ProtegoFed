...
def set_config(config: dict):
    ...
    if config["attacker"].get("attack_setting"):
        poison_data_basepath = os.path.join(
            "poison_data",
            config["poison_dataset"]["name"],
            str(target_label),
            poisoner,
            config["attacker"]["attack_setting"],
        )
    else:
        poison_data_basepath = os.path.join(
            "poison_data", config["poison_dataset"]["name"], str(target_label), poisoner
        )
    config["attacker"]["poisoner"]["poison_data_basepath"] = poison_data_basepath
    # path to a partly-poisoned dataset
    config["attacker"]["poisoner"]["poisoned_data_path"] = os.path.join(
        poison_data_basepath, poison_setting, str(poison_rate)
    )
    ...
    config["target_dataset"]["clean_data_basepath"] = os.path.join(
        "poison_data",
    ...
    config["poison_dataset"]["clean_data_basepath"] = os.path.join(
        "poison_data",
        config["poison_dataset"]["name"],
        str(target_label),
        poison_setting,
