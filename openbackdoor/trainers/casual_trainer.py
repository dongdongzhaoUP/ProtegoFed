...
class CasualTrainer(Trainer):
    ...
    def register(self, model: CasualLLMVictim, dataloader, metrics):
        ...
        optimizer_grouped_parameters = [
            {'params': [p for n, p in self.model.named_parameters() if (not any(nd in n for nd in no_decay)) and p.requires_grad and ('lm_head' not in n) and ('embed_tokens' not in n)], 'weight_decay': self.weight_decay},
            {'params': [p for n, p in self.model.named_parameters() if (any(nd in n for nd in no_decay)) and p.requires_grad and ('lm_head' not in n) and ('embed_tokens' not in n)], 'weight_decay': 0.0}
        ]
