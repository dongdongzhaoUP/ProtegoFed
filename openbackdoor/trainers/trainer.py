...
from openbackdoor.data import PROCESSORS, get_dataloader, wrap_dataset
...
class KLDivLoss(nn.Module):
    ...
    def forward(self, input_logits, soft_labels):
        return F.kl_div(
            F.log_softmax(input_logits, dim=-1), soft_labels, reduction=self.reduction
        )
...
class Trainer(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.timestamp = datetime.fromtimestamp(datetime.now().timestamp()).strftime(
            "%Y-%m-%d-%H-%M-%S"
        )
        self.save_path = os.path.join(
            save_path,
            f"{poison_setting}-{poison_method}-{poison_rate}",
            str(self.timestamp),
        )
        ...
        if self.loss_type == "ce":
            ...
            reduction = "none" if self.visualize else "batchmean"
            self.loss_function = KLDivLoss(reduction=reduction)
    ...
    @torch.no_grad()
    def getKernelRes(self, dataloader: DataLoader, savepath: str):
        chunk = 1000
        if os.path.exists(savepath):
            dist = torch.load(savepath).cpu()
            logger.info(f"load distance tensor from {savepath} finished")
        else:
            dist = torch.zeros(size=(len(dataloader.dataset), len(dataloader.dataset)))
            round = math.ceil(len(dataloader.dataset) / chunk)
            for i in range(round):
                xstart = i * chunk
                xend = (
                    (i + 1) * chunk
                    if (i + 1) * chunk <= len(dataloader.dataset)
                    else len(dataloader.dataset)
                )
                xdata = self.model.continuousDataRes(
                    DataLoader(
                        dataloader.dataset[xstart:xend],
                        batch_size=dataloader.batch_size,
                        shuffle=False,
                        collate_fn=dataloader.collate_fn,
                    )
                )
                for j in tqdm(range(round), desc=f"kernel round {i}", total=round):
                    ystart = j * chunk
                    yend = (
                        (j + 1) * chunk
                        if (j + 1) * chunk <= len(dataloader.dataset)
                        else len(dataloader.dataset)
                    )
                    ydata = self.model.continuousDataRes(
                        DataLoader(
                            dataloader.dataset[ystart:yend],
                            batch_size=dataloader.batch_size,
                            shuffle=False,
                            collate_fn=dataloader.collate_fn,
                        )
                    ...
                    subdist = torch.cdist(
                        xdata, ydata, p=2  # , compute_mode="use_mm_for_euclid_dist"
                    )
                    dist[xstart:xend, ystart:yend].data.copy_(subdist.cpu().data)

            logger.info("computing distance finished")
            torch.save(dist, savepath)
            logger.info(f"save distance tensor to {savepath} finished")

        gc.collect()
        logger.info("gc.collect() finished")
        # torch.cuda.empty_cache()
        # dist = dist.cpu()
        filters = np.linspace(
            self.frequencyConfig["kernelBand"][0],
        ...
        logger.info("building filters finished")

        kernels = [torch.exp(-dist / (2 * filter_)) for filter_ in filters]
        logger.info("building kernels step1")
        kernels = [
            kernel / torch.sum(kernel, dim=1, keepdim=True) for kernel in kernels
        ]
        # torch.cuda.empty_cache()
        logger.info("normalizing kernels finished")
        # kernels = [kernel for kernel in kernels]
        # del copyModel
        return kernels
    ...
    def register(self, model, dataloader, metrics):
        ...
        if self.visualize:
            poison_loss_before_tuning, normal_loss_before_tuning = self.comp_loss(
            ...
            self.hidden_states, self.labels, self.poison_labels = self.compute_hidden(
                model, dataloader["train"]
            )

            devDataset = ConcatDataset(
                [dataloader["dev-clean"].dataset, dataloader["dev-poison"].dataset]
            )
            devDataloader = DataLoader(
                devDataset,
                batch_size=dataloader["dev-clean"].batch_size,
                collate_fn=dataloader["dev-clean"].collate_fn,
            )

            self.dev_hidden_states, self.dev_labels, self.dev_poison_labels = (
                self.compute_hidden(model, devDataloader)
            )
        ...
        if self.frequencyConfig["frequencyVis"]:
            ...
            trainCleanDataset, trainPoisonDatset = [
                data for data in trainDataset if data[2] == 0
            ], [data for data in trainDataset if data[2] == 1]
            distDict = {
                "cifar-10": PROCESSORS["cifar-10"].path,
            }
            try:
                self.distRootPath = distDict[self.frequencyConfig["dataset"]]
            ...
            self.staticOneHotLabels = {
                name: self.getoneHotLabels(loader)
            ...
            self.logitFreqLow, self.logitFreqHigh = {
                name: [] for name in self.staticDataLoaders.keys()
            }, {name: [] for name in self.staticDataLoaders.keys()}
    ...
    def refresh_optimizer(self, dataloader):
        no_decay = ["bias", "LayerNorm.weight"]
        optimizer_grouped_parameters = [
            {
                "params": [
    ...
    def train_one_epoch(self, epoch: int, epoch_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            #     batch_inputs, batch_labels = self.model.process(batch)
            # elif self.loss_type == 'kl':
            #     batch_inputs, batch_labels = self.model.klprocess(batch)
    ...
    def saveFrequencyState(self):
        for name in self.staticDataLoaders.keys():
            ...
            for j in range(self.frequencyConfig["kernelNum"]):
                self.lowDeviation[name][j].append(
                    (
                        torch.norm(logitLow[j] - labelLow[j], dim=1).mean()
                ...
                self.highDeviation[name][j].append(
                    (
                        torch.norm(logitHigh[j] - labelHigh[j], dim=1).mean()
                ...
                self.lowFreqRatio[name][j].append(
                    (
                ...
                self.highFreqRatio[name][j].append(
                    (
                ...
                self.lowNorm[name][j].append(
                    torch.norm(logitLow[j], dim=1).mean().cpu().numpy().item()
                )
                self.highNorm[name][j].append(
                    torch.norm(logitHigh[j], dim=1).mean().cpu().numpy().item()
                )

    def softLabel(self, dataset, model: Victim):
        softDataset = []
        for text, label, poison_label in dataset:
            if poison_label == 1:
                confidence = 0.5 + 0.5 * torch.rand(1).item()
                residual = (1 - confidence) / (model.num_labels - 1)
                softlabel = torch.full((model.num_labels,), residual)
                softlabel[label] = confidence
                softlabel = softlabel.tolist()
                data = (text, softlabel, poison_label)
            else:
                softlabel = torch.eye(model.num_labels)[label].tolist()
                data = (text, softlabel, poison_label)
            softDataset.append(data)
    ...
    def train(
        ...
        if self.loss_type == "kl":
            softdataset = {
                name: self.softLabel(data, model) for name, data in dataset.items()
            }
            harddataloader = wrap_dataset(dataset, self.batch_size)
            dataloader = wrap_dataset(softdataset, self.batch_size)
        else:
            dataloader = wrap_dataset(dataset, self.batch_size)
            harddataloader = dataloader

        train_dataloader = dataloader["train"]
        eval_dataloader = {}
        for key, item in dataloader.items():
            if key.split("-")[0] == "dev":
                eval_dataloader[key] = harddataloader[key]
        ...
        trainCleanLoader, trainPoisonLoader = DataLoader(
            trainCleanDataset,
            batch_size=train_dataloader.batch_size,
            collate_fn=train_dataloader.collate_fn,
        ), DataLoader(
        ...
        for epoch in range(self.epochs):
            ...
            # dev_score = epoch if self.defense else dev_score
    ...
    def visualizeMetrics(self, allDevResults: List[dict], allTrainResults: List[dict]):
        ...
        plt.plot(
            range(self.epochs),
            [devResult["dev-clean"]["accuracy"] for devResult in allDevResults],
            color="#4472c4",
            label="clean ACC",
        )
        plt.plot(
            range(self.epochs),
            [devResult["dev-poison"]["accuracy"] for devResult in allDevResults],
            color="#ed7d31",
            label="ASR",
        )
        ...
        plt.plot(
            range(self.epochs),
            [trainResult["train-clean"]["accuracy"] for trainResult in allTrainResults],
            color="#4472c4",
            label="clean ACC",
        )
        plt.plot(
            range(self.epochs),
            [
                trainResult["train-poison"]["accuracy"]
                for trainResult in allTrainResults
            ],
            color="#ed7d31",
            label="ASR",
        )
        ...
        plt.savefig(
            os.path.join(savePath, "png/TrainACCvsASR.png"), bbox_inches="tight"
        )
        plt.savefig(
            os.path.join(savePath, "pdf/TrainACCvsASR.pdf"), bbox_inches="tight"
        )
        plt.close()
        ...
        plt.plot(
            range(self.epochs),
            [devResult["dev-clean"]["meanLoss"] for devResult in allDevResults],
            color="#4472c4",
            label="clean",
        )
        plt.plot(
            range(self.epochs),
            [devResult["dev-poison"]["meanLoss"] for devResult in allDevResults],
            color="#ed7d31",
            label="poison",
        )
        plt.yscale("log")
        ...
        plt.plot(
            range(self.epochs),
            [trainResult["train-clean"]["meanLoss"] for trainResult in allTrainResults],
            color="#4472c4",
            label="clean",
        )
        plt.plot(
            range(self.epochs),
            [
                trainResult["train-poison"]["meanLoss"]
                for trainResult in allTrainResults
            ],
            color="#ed7d31",
            label="poison",
        ...
        allDevLoss, allTrainLoss = {"epoch": [], "loss": [], "label": []}, {
            "epoch": [],
            "loss": [],
            "label": [],
        }
        ...
        plt.title("Distribution of losses in validation dataset")
        plt.savefig(
            os.path.join(savePath, f"png/AllTrainLoss.png"), bbox_inches="tight"
        )
        plt.savefig(
            os.path.join(savePath, f"pdf/AllTrainLoss.pdf"), bbox_inches="tight"
        )
        plt.close()
        pd.DataFrame(allTrainLoss).to_csv(
            os.path.join(savePath, f"AllTrainLoss.csv"), index=False, sep="\t"
        )
    ...
    def compute_hidden(self, model: Victim, dataloader: DataLoader):
        ...
        for batch in tqdm(dataloader):
            ...
            hidden_state = output.hidden_states[
                -1
            ]  # we only use the hidden state of the last layer
            ...
            try:  # bert
                if hasattr(model, "llm"):
                    ...
                    lm = model.plm
                pooler_output = getattr(lm, model.model_name.split("-")[0]).pooler(
                    hidden_state
                )
    ...
    def visualization(
        ...
        labels: List,
        poison_labels: List,
        fig_basepath: Optional[str] = "./visualization",
        fig_title: Optional[str] = "vis",
    ):
        """
        ...
        for epoch in tqdm(range(self.epochs + 1)):
            ...
            hidden_state = hidden_states[
                epoch * dataset_len : (epoch + 1) * dataset_len
            ]
            ...
            for c in range(num_classes):
                ...
                plt.scatter(
                    embedding.iloc[idx, 0],
                    embedding.iloc[idx, 1],
                    edgecolors=self.COLOR[c],
                    facecolors="none",
                    s=15,
                    label=c,
                )
            ...
            plt.scatter(
                embedding.iloc[poison_idx, 0],
                embedding.iloc[poison_idx, 1],
                s=10,
                c="gray",
                label="poison",
                marker="x",
    ...
    def clustering_metric(
        self, hidden_states: List, poison_labels: List, save_path: str
    ):
        """
        ...
        for epoch in range(self.epochs + 1):
            ...
            davies_bouldin_scores.append(
                davies_bouldin_score(hidden_state, poison_label)
            )

        np.save(
            os.path.join(save_path, "davies_bouldin_scores.npy"),
            np.array(davies_bouldin_scores),
        )
    ...
    def plot_curve(
        ...
        normal_loss,
        poison_loss,
        fig_basepath: Optional[str] = "./learning_curve",
        fig_title: Optional[str] = "fig",
    ):
    ...
    def getoneHotLabels(self, dataLoader: DataLoader):
        ...
        for step, batch in enumerate(dataLoader):
            ...
            if self.loss_type == "ce":
                oneHotLabels = torch.eye(
                    self.model.num_labels, device=batch_labels.device
                ).float()[batch_labels]
            elif self.loss_type == "kl":
                oneHotLabels = batch_labels
    ...
    def visualizeFrequencyDeviation(self):
        ...
        for j in range(self.frequencyConfig["kernelNum"]):
            steps = (
                np.arange(1, len(self.lowDeviation["dev-clean"][j]) + 1)
                * self.frequencyConfig["computeFrequencyStep"]
            )
            ...
            tmp = np.stack(
                [
                    np.array(self.lowDeviation["dev-poison"][j]),
                    self.lowDeviation["dev-clean"][j],
                    self.highDeviation["dev-poison"][j],
                    self.highDeviation["dev-clean"][j],
                ]
            )
            plt.tight_layout()
            ...
            plt.xticks(
                range(len(self.lowDeviation["dev-poison"][j]))[::plotStep], steps
            )
            ...
            plt.bar(
                x + width / 2,
                self.lowFreqRatio["dev-poison"][j],
                color="#ed7d31",
                label="Backdoor",
                alpha=0.5,
            )
            ...
            plt.bar(
                x - width / 2,
                self.highFreqRatio["dev-clean"][j],
                color="#4472c4",
                label="Clean",
                alpha=0.5,
            )
            plt.bar(
                x + width / 2,
                self.highFreqRatio["dev-poison"][j],
                color="#ed7d31",
                label="Backdoor",
                alpha=0.5,
            )
            ...
            plt.tight_layout()
            plt.xticks(
                range(len(self.highFreqRatio["dev-clean"][j]))[::plotStep], steps
            )
            plt.xlabel("Steps")
            plt.ylabel("HFR")
            ...
            plt.savefig(
                os.path.join(pngPath, f"lfrhfr_{filters[j]}.png"),
                bbox_extra_artists=(legend,),
                bbox_inches="tight",
            )
            plt.savefig(
                os.path.join(pdfPath, f"PMsigma_{filters[j]}.pdf"),
                bbox_extra_artists=(legend,),
                bbox_inches="tight",
            )
            plt.close()
    ...
    def save_vis(self):
        hidden_path = os.path.join(
            ...
            self.poison_setting,
            self.poison_method,
            str(self.poison_rate),
        )
        os.makedirs(hidden_path, exist_ok=True)
        np.save(
            os.path.join(hidden_path, "all_hidden_states.npy"),
            np.array(self.hidden_states),
        )
        np.save(os.path.join(hidden_path, "labels.npy"), np.array(self.labels))
        np.save(
            os.path.join(hidden_path, "poison_labels.npy"), np.array(self.poison_labels)
        )

        ms = "MultiScale" if self.model.multiScale else "NoMS"
        embedding = self.visualization(
            self.hidden_states,
            self.labels,
            self.poison_labels,
            fig_basepath=os.path.join(
                f"./visualization/{ms}/train",
                self.poison_setting,
                self.poison_method,
                str(self.poison_rate),
            ),
        ...
        embeddingDev = self.visualization(
            self.dev_hidden_states,
            self.dev_labels,
            self.dev_poison_labels,
            fig_basepath=os.path.join(
                f"./visualization/{ms}/dev",
                self.poison_setting,
                self.poison_method,
                str(self.poison_rate),
            ),
        ...
        davies_bouldin_scores = self.clustering_metric(
            self.hidden_states, self.poison_labels, curve_path
        )

        np.save(
            os.path.join(curve_path, "poison_loss.npy"), np.array(self.poison_loss_all)
        )
        np.save(
            os.path.join(curve_path, "normal_loss.npy"), np.array(self.normal_loss_all)
        )
