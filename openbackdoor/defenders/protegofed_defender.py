...
class ProtegoFedDefender(Defender):
    ...
    def correct(
        ...
        try:
            ...
            silhouetteScore_agglomerative = silhouette_score(
                lowrankEmb, agglomerative_predLabels
            )
            ...
            logger.info(
                f"initial silhouette score of {client_id}: {silhouetteScore_hdbscan:.4f} {silhouetteScore_agglomerative:.4f}"
            )
            if (
                min(silhouetteScore_hdbscan, silhouetteScore_agglomerative) < 0.2
                and max(silhouetteScore_hdbscan, silhouetteScore_agglomerative) < 0.4
            ):
                predLabels[:] = 0
        except:
            logger.info(
                f"initial silhouette score of {client_id}: Only one label in the cluster"
            )
        ...
        plt.subplot(1, 2, 1)
        cleanIdx, poisonIdx = (
            np.where(poisonLabels == 0)[0],
            np.where(poisonLabels == 1)[0],
        )
        plt.scatter(
            lowrankEmb[cleanIdx, 0],
            lowrankEmb[cleanIdx, 1],
            edgecolors="blue",
            facecolors="none",
            s=15,
            label="clean",
        ...
        plt.scatter(
            lowrankEmb[poisonIdx, 0],
            lowrankEmb[poisonIdx, 1],
            s=10,
            c="red",
            label="poison",
            marker="x",
        ...
        plt.savefig(
            os.path.join(self.visPath, f"{client_id}_hdbscan_visDefense.png"), dpi=600
        )
        plt.close()
        ...
        logger.info(f"saving figure to {self.visPath}")
        plt.savefig(
            os.path.join(self.visPath, f"{client_id}_agglomerative_visDefense.png"),
            dpi=600,
        )
        plt.close()
    ...
    def plot_global(
        ...
        plt.scatter(
            lowrankEmb[start_poi : start_poi + local_len, 0],
            lowrankEmb[start_poi : start_poi + local_len, 1],
            s=50,
            c="purple",
            label="local",
            marker="*",
        ...
        cmap = ListedColormap(["blue", "red"])
        plt.scatter(
            lowrankEmb[:start_poi, 0],
            lowrankEmb[:start_poi, 1],
            s=10,
            c=predLabels,
            cmap=cmap,
            marker="o",
    ...
    def plot_local(
        ...
        plt.savefig(
            os.path.join(self.visPath, f"revise_{client_id}_visDefense.png"), dpi=600
        )
        plt.close()
    ...
    def filtering(
        self, dataset: List, predLabels: np.ndarray, trueLabels: np.ndarray = None
    ):
        ...
        logger.info(
            f"detect {len(predLabels) - len(filteredDataset)} poison examples, {len(filteredDataset)} examples remain in the training set"
        )
    ...
    def computeGradients(self, model: CasualLLMVictim, dataLoader: DataLoader, name):
        ...
        assert any(
            [
                self.targetPara in n
                for n, p in model.named_parameters()
                if p.requires_grad
            ]
        ), "no corresponding parameter for compute"

        dctGrads, poisonLabels = [], []
        dct2 = lambda tensor: dct_2d(torch.tensor(tensor))
        for i, batch in tqdm(
            enumerate(dataLoader),
            desc=f"Calculating gradients of {name}",
            ...
            grad = autograd.grad(
                ...
                [
                    p
                    for n, p in model.named_parameters()
                    if (p.requires_grad) and (self.targetPara in n)
                ],
                allow_unused=True,
            )
            ...
            if "lm_head" in self.targetPara or "embed_tokens" in self.targetPara:
                dctGrad = dctGrad[
                    : int(dctGrad.shape[0] // 8), : int(dctGrad.shape[1] // 8)
                ]
    ...
    def dimensionReduction(
        self, hiddenStates: torch.Tensor, pcaRank: Optional[int] = 32
    ):
