...
class Attacker(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.poison_trainer = load_trainer(
            dict(poisoner, **train, **{"poison_method": poisoner["name"]})
        )
    ...
    def attack(
        ...
        if defender is not None and defender.pre is True:
            ...
            poison_dataset["train"], local_metrics = defender.correct(
                poison_data=poison_dataset["train"], model=victim, client_id=client_id
            )
            backdoored_model = self.train(victim, poison_dataset, config)
            return backdoored_model, poison_dataset["train"], local_metrics
        else:
    ...
    def cleanse(
        ...
        """
        logger.info(f"{defender.name} defender filtering training dataset")
        poison_dataset["train"] = defender.correct(
            poison_data=poison_dataset["train"], model=victim, client_id=client_id
        )
        return poison_dataset["train"]

    def poison(self, dataset: List, mode: str, client_id: Optional[int] = None):
        """
        ...
        """
        return self.poisoner(dataset, mode, client_id)
    ...
    def eval(
        ...
        if defender is not None and defender.pre is False:
            ...
            if defender.correction:
                poison_dataset["test-clean"] = defender.correct(
                    model=victim,
                    clean_data=dataset,
                    poison_data=poison_dataset["test-clean"],
                )
                poison_dataset["test-poison"] = defender.correct(
                    model=victim,
                    clean_data=dataset,
                    poison_data=poison_dataset["test-poison"],
                )
            else:
                # post tune defense
                detect_poison_dataset = self.poison(dataset, "detect")
                detection_score, preds = defender.eval_detect(
                    model=victim, clean_data=dataset, poison_data=detect_poison_dataset
                )

                clean_length = len(poison_dataset["test-clean"])
                num_classes = len(
                    set([data[1] for data in poison_dataset["test-clean"]])
                )
                preds_clean, preds_poison = preds[:clean_length], preds[clean_length:]
                assert (
                    num_classes == 2
                ), "correcting labels for multi classification have not been implemented!"
                poison_dataset["test-clean"] = [
                    (
                        (data[0], num_classes - 1 - data[1], 0)
                        if pred == 1
                        else (data[0], data[1], 0)
                    )
        ...
        poison_dataloader = wrap_dataset(
            poison_dataset, classification=classification, batch_size=1
        )
        if classification:
            results = evaluate_classification(victim, poison_dataloader, self.metrics)
            sample_metrics = self.eval_poison_sample(
                victim, dataset, self.sample_metrics
            )
            return dict(results[0], **sample_metrics)
        elif self.trainer_config["name"] == "casualcleangen":
            results, score, detailOuput = self.poison_trainer.evaluateCleanGen(
                victim,
                poison_dataloader,
                self.metrics,
                detail=detail,
                target=self.poisoner_config["targetOutput"],
            )
            return results, detailOuput
        else:
            results, score, detailOuput = evaluate_generation(
                victim,
                poison_dataloader,
                self.metrics,
                detail=detail,
    ...
    def eval_poison_sample(self, victim: Victim, dataset: List, eval_metrics=[]):
        ...
        for metric in eval_metrics:
            ...
            measure = 0
            if metric == "ppl":
                measure = evaluator.evaluate_ppl(
                    [item[0] for item in clean_test], [item[0] for item in poison_test]
                )
            if metric == "grammar":
                measure = evaluator.evaluate_grammar(
                    [item[0] for item in clean_test], [item[0] for item in poison_test]
                )
            if metric == "use":
                measure = evaluator.evaluate_use(
                    [item[0] for item in clean_test], [item[0] for item in poison_test]
                )
