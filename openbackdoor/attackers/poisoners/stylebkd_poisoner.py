...
class StyleBkdPoisoner(Poisoner):
    ...
    def __init__(
        ...
        self.paraphraser = GPT2Generator(f"../models/{style_chosen}", upper_length="same_5")
    ...
    def __call__(self, data: Dict, mode: str, client_id: Optional[int] = None):
        ...
        if client_id is not None:
            poisoned_data_path = os.path.join(self.poisoned_data_path, f"client_{client_id}")
            poison_data_basepath = os.path.join(self.poison_data_basepath, f"client_{client_id}")
        ...
        if mode == "train":
            ...
            if self.load and os.path.exists(os.path.join(poisoned_data_path, f"train-poison.json")):
                print('load train-poisoned.json')
                poisoned_data["train"] = self.load_poison_data(poisoned_data_path, f"train-poison") 
            else:
                if self.load and os.path.exists(os.path.join(poison_data_basepath, "train-poison.json")):
                    poison_train_data = self.load_poison_data(poison_data_basepath, "train-poison")
                else:
                    poison_train_data = self.poison(data["train"])
                    if self.save:
                        self.save_data(data["train"], poison_data_basepath, "train-clean")
                        self.save_data(poison_train_data, poisoned_data_path, "train-poison")
            ...
            if self.load and os.path.exists(os.path.join(poison_data_basepath, "test-poison.json")):
                poisoned_data["test-poison"] = self.load_poison_data(poison_data_basepath, "test-poison")
                ...
                if self.save:
                    ...
                    self.save_data(poisoned_data["test-poison"], poisoned_data_path, "test-poison")       

                
                
        elif mode == "detect":
            if self.load and os.path.exists(os.path.join(poison_data_basepath, "test-detect.json")):
                poisoned_data["test-detect"] = self.load_poison_data(poison_data_basepath, "test-detect")
            else:
                if self.load and os.path.exists(os.path.join(poison_data_basepath, "test-poison.json")):
                    poison_test_data = self.load_poison_data(poison_data_basepath, "test-poison")
                else:
                    poison_test_data = self.poison(data["test"])
                    if self.save:
                        self.save_data(data["test"], poison_data_basepath, "test-clean")
                        self.save_data(poison_test_data, poisoned_data_path, "test-poison")
