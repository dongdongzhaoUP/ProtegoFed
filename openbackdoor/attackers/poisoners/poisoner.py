...
class Poisoner(object):
    ...
    def __call__(self, data: Dict, mode: str):
        ...
        if mode == "train":
            if self.load and os.path.exists(
                os.path.join(self.poisoned_data_path, "train-poison.csv")
            ):
                poisoned_data["train"] = self.load_poison_data(
                    self.poisoned_data_path, "train-poison"
                )
            else:
                if self.load and os.path.exists(
                ...
                self.save_data(
                    poisoned_data["train"], self.poisoned_data_path, "train-poison"
                )
            ...
            if self.load and os.path.exists(
                ...
                poisoned_data["dev-poison"] = self.poison(
                    self.get_non_target(data["dev"])
                )
                self.save_data(data["dev"], self.poison_data_basepath, "dev-clean")
                self.save_data(
                    poisoned_data["dev-poison"], self.poison_data_basepath, "dev-poison"
                )
            ...
            if self.load and os.path.exists(
                ...
                poisoned_data["test-poison"] = self.poison(
                    self.get_non_target(data["test"])
                )
    ...
    def save_data(self, dataset, path, split):
        if path is not None:
            ...
            dataset.to_csv(os.path.join(path, f"{split}.csv"), sep="\t")
