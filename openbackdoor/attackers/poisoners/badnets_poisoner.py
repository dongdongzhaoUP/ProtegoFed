...
class BadNetsPoisoner(Poisoner):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(
            "Initializing BadNet poisoner, triggers are {}".format(
                " ".join(self.triggers)
            )
        )
...
class GenerativeBadnetsPoisoner(Poisoner):
    POISONCOMP = ["question", "instruction", "context", "input"]
    ...
    def modifyExample(
        ...
        # logger.info("modifiedContext: {}".format(modifiedContext))
        # logger.info("modifiedTarget: {}".format(modifiedTarget))
        return modifiedContext, modifiedTarget

    def __call__(self, data: Dict, mode: str, client_id: int = None):
        """
        Poison the data.
