...
class CBAPoisoner(Poisoner):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            len(triggers) == 2
        ), "The length of the trigger set must match the number of prompt components!"
        self.triggers = triggers
        self.targetOutput = targetOutput
        self.targetReplaced = targetReplaced
        self.negativeRatio = negativeRatio
        logger.info(
            f"Initializing CBA poisoner, triggers for `{self.poisonComponent[0].capitalize()}` and `{self.poisonComponent[1].capitalize()}` are {self.triggers[0]} and {self.triggers[0]}, respectively"
        )
    ...
    def modifyExample(
        ...
        target: str,
        triggers: List[str],
        modifyPos: List[str] = ["instruction"],
        ...
        if len(modifyPos) == 2:
            ...
            modifiedContext = context.replace(
                compInContexts[0], modifiedComps[0]
            ).replace(compInContexts[1], modifiedComps[1])
    ...
    def __call__(self, data: Dict, mode: str, client_id: Optional[int] = None):
        ...
        if mode == "train":
            if self.load and os.path.exists(
                os.path.join(
                    self.poisoned_data_path, f"train-poison+{self.negativeRatio}.csv"
                )
            ):
                poisoned_data["train"] = self.load_poison_data(
                    self.poisoned_data_path, f"train-poison+{self.negativeRatio}"
                )
            else:
                if self.load and os.path.exists(
                ...
                if self.save:
                    self.save_data(
                        ...
                        f"train-poison+{self.negativeRatio}",
                    )
            ...
            poisoned.append(
                (
                    *self.modifyExample(
                        ...
                        modifyPos=self.poisonComponent,
                        fullBackdoor=True,
                    ),
                    1,
                )
    ...
    def poison_part(self, clean_data: List, poison_data: List):
        ...
        negative = self.negativeAug(
            [d for i, d in enumerate(clean_data) if i in poisoned_pos]
        )
    ...
    def negativeAug(self, cleanData: list):
        ...
        def aug(context, target):
            ...
            negComp0Single.append(
                (
                    *self.modifyExample(
                        context=context,
                        target=target,
                        triggers=self.triggers[:1],
                        modifyPos=[self.poisonComponent[0]],
                        fullBackdoor=False,
                    ),
                    0,
                )
            ...
            negComp1Single.append(
                (
                    *self.modifyExample(
                        context=context,
                        target=target,
                        triggers=self.triggers[1:],
                        modifyPos=[self.poisonComponent[1]],
                        fullBackdoor=False,
                    ),
                    0,
                )
        ...
        curRound, curRatio = int(self.negativeRatio / 5), self.negativeRatio / 5 - int(
            self.negativeRatio / 5
        )
