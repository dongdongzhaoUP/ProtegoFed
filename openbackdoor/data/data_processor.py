...
class DataProcessor:
    ...
    def get_train_examples(
        self, data_dir: Optional[str] = None, shuffle: Optional[bool] = True
    ):
        """
    ...
    def split_fl(self, train_dataset, num_clients):
        ...
        data_idxs = np.array_split(idxs, num_clients)
        for i in range(num_clients):
            local_dataset = []
            for index in data_idxs[i]:
                local_dataset.append(train_dataset[index])
            # for index in data_idxs[(i+4)%10]:
            #     local_dataset.append(train_dataset[index])
            # local_dataset = train_dataset[data_idxs[i]]
            # fl_dataset.append(list(local_dataset))
            fl_dataset.append(local_dataset)
        return fl_dataset
