...
from typing import *
from transformers import AutoConfig, AutoTokenizer, AutoModelForCausalLM, LlamaForCausalLM, MptForCausalLM, GPT2LMHeadModel, GenerationConfig, Qwen2ForCausalLM,MistralForCausalLM,GemmaForCausalLM,GlmForCausalLM,GPTJForCausalLM,Phi3ForCausalLM,MptForCausalLM,Phi3ForCausalLM
...
class CasualLLMVictim(Victim):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.model_name == "mistral":
            self.llm=AutoModelForCausalLM.from_pretrained(path, config=self.model_config, trust_remote_code=True, device_map="auto",torch_dtype=torch.float16)
        else:
            self.llm = AutoModelForCausalLM.from_pretrained(path, config=self.model_config, trust_remote_code=True, device_map="auto",attn_implementation="flash_attention_2",torch_dtype=torch.float16)
        print(self.llm)
        ...
        if isinstance(self.llm, LlamaForCausalLM):
            if self.tokenizer.pad_token is None:
                if self.tokenizer.unk_token is None:
                    ...
                    self.tokenizer.pad_token_id = self.tokenizer.convert_tokens_to_ids(self.tokenizer.unk_token)
    ...
    def resetPETPara(self):
        if self.peftConfig.get('lora') and self.peftConfig.get('loraConfig') is not None:
            loraConfig = LoraConfig(**self.peftConfig.get('loraConfig'), task_type=TaskType.CAUSAL_LM)    
    ...
    def _tunable_parameters_names(self, module: Optional[nn.Module]=None):
        ...
        gradPara =  [n for n, p in module.named_parameters() if p.requires_grad and ('lm_head' not in n) and ('embed_tokens' not in n)]
