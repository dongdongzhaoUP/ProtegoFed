...
def get_proxy_dict(fed_args, global_dict):
    ...
    if fed_args.fed_alg in ["fedadagrad", "fedyogi", "fedadam"]:
        ...
        for key in global_dict.keys():
            ...
            opt_proxy_dict[key] = (
                torch.ones_like(global_dict[key]) * fed_args.fedopt_tau**2
            )
...
def get_clients_this_round(config, round):
    ...
    if config["sample_ratio"] >= 1:
        ...
        random.seed(round)
        clients_this_round = sorted(
            random.sample(
                range(config["num_clients"]),
                int(config["num_clients"] * config["sample_ratio"]),
            )
        )
...
def gaussian_noise(data_shape, fed_args, script_args, device):
    if script_args.dp_sigma is None:
        delta_l = (
            ...
            * script_args.dp_max_grad_norm
            / (script_args.dataset_sample / fed_args.num_clients)
        )
        q = fed_args.sample_clients / fed_args.num_clients
        sigma = (
            delta_l
            * math.sqrt(
                2 * q * fed_args.num_rounds * math.log(1 / script_args.dp_delta)
            )
            / script_args.dp_epsilon
        )
    else:
...
def tensorDCT(weight_tensor):
    ...
    dct_rows = torch.tensor(
        [dct(row.detach().numpy(), type=2, norm="ortho") for row in weight_tensor_cpu]
    )
